const clients={
Shreyas:{id:"BRCLT-45871293",password:"SHR129",balance:4500000000,limit:1000000,pending:3855},
Zain:{id:"BRCLT-797555",password:"ZAI129",balance:11000000000,limit:2500000,pending:3855},
...
Tanveer:{id:"BRCLT-TANVEER",password:"TAN129",balance:550000,limit:100,pending:1500}
...
function renderActivity(){document.getElementById("activity").innerHTML=`<div class="activity"><b>Paper order filled</b><div class="muted">NIFTY · BUY · ₹1,20,000</div><small>Today, 10:20</small></div><div class="activity"><b>Payment pending</b><div class="muted">₹2,500 · MSN-2508-001</div><small>Today, 08:40</small></div><div class="activity"><b>Withdrawal recorded</b><div class="muted">₹75,000 · MSN-WD-8841</div><small>21 Aug 2026</small></div>`}
let chartInstance, selectedInstrument="NIFTY", tradeSide="BUY";
const prices={"IDFC NOVA":1284.20," SBI Bank":842.60,"Atlas Motors":516.35,"HDB":2108.75};
const changes={"NIFTY":"+2.14%"," PNJ Bank":"+0.81%","Atlas Motors":"-0.42%","IDFC":"+1.32%"};
...
function postComment(){let v=document.getElementById("comment").value.trim();if(!v)return;let d=document.createElement("div");d.className="message";d.innerHTML=`<b>Client note</b><p>${v.replace(/[<>&]/g,m=>({"<":"&lt;",">":"&gt;","&":"&amp;"}[m]))}</p><small>Just now ·</small>`;document.getElementById("comments").prepend(d);document.getElementById("comment").value=""}
function downloadCSV(type){const rows=type==="account"?"Date,Type,Reference,Amount,Status\\n2026-08-30,Payment,SIM-2508-001,2500,Pending\\n2026-08-29,Withdrawal,SIM-WD-8841,35000,Approved":"Date,Instrument,Side,Quantity,Price\\n2026-08-30,MSN Energy,BUY,10,1284.20\\n2026-08-28,Bank,SELL,20,842.60";const blob=new Blob([rows],{type:"text/csv"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`msn-brock-${type}-.csv`;a.click()}
...
document.addEventListener("ContentLoaded",renderLoginClients);

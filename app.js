...
$("loginBtn").onclick=()=>{let c=CLIENTS.find(x=>x[1]==$("clientId").value);if($("password").value!==c[2]){$("loginError").textContent="Invalid password.";return} $("loginError").textContent="";load(c)};
...
$("policyBtn").onclick=()=>{if(!$("withdrawAmount").value||!$("bankAccount").value){alert("Enter the withdrawal amount and  bank account.");return}$("policyBox").classList.remove("hidden")};
...
$("withdrawBtn").onclick=()=>{let a=Number($("withdrawAmount").value),u=$("utr").value.trim();if(!u){alert("Enter a  UTR/reference.");return}state.withdrawals.push({amount:a,utr:u,time:new Date().toLocaleString()});save();renderWithdrawals();alert(" withdrawal submitted.");$("withdrawAmount").value="";$("bankAccount").value="";$("utr").value=""};
function renderWithdrawals(){$("withdrawHistory").innerHTML=state.withdrawals.length?state.withdrawals.slice().reverse().map(x=>`<div class="withdraw-item"><b>${money(x.amount)}</b> •  • ${x.utr}<br><small>${x.time}</small></div>`).join(""):`<div class="history empty">No withdrawals yet.</div>`}
